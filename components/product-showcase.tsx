...
export default function ProductShowcase() {
  ...
  useEffect(() => {
    ...
    const fetchImages = async () => {
      // Fetch the single showcase document
      const result = await client.fetch(`*[_type == "showcase"][0]`)
      setSanityData(result)
    }
    fetchImages()
  }, [])

  // Helper to safely get the Sanity URL or fallback to your local path
  const getLipstickImg = () => {
    if (sanityData?.lipstickImage) return urlFor(sanityData.lipstickImage).width(1000).url();
    return "/lipstick/pic12.jpg"; // Your original local fallback
  }

  const getSkincareImg = () => {
    if (sanityData?.skincareImage) return urlFor(sanityData.skincareImage).width(1000).url();
    return "/skincare/pic15.jpg"; // Your original local fallback
  }
